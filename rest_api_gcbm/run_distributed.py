...
EXAMPLE_STAGING_ROOT = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "local_files"))
EXAMPLE_FILESERVER_ROOT = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "remote_files"))
...
def run_block(title, tile_idx, block_idx):
    ...
    landscape_config_file = find_config_file(block_working_dir, "LocalDomain", "landscape")
    with update_json_file(landscape_config_file) as study_area_config:
        ...
        landscape_config["blocks"] = [{"tile_index": tile_idx, "block_index": block_idx}]
        if "tiles" in landscape_config:
            del landscape_config["tiles"]

    output_dir = os.path.join(working_dir, "output")
    os.makedirs(output_dir, exist_ok=True)
    batch_config_file = (glob(os.path.join(block_working_dir, "*.cfg")) \
        + glob(os.path.join(block_working_dir, "gcbm_config.json")))[0]

    gcbm_args = [
        "/some/server-side/flint/moja.cli",
        "--config_file" if batch_config_file.endswith("cfg") else "--config", batch_config_file,
    ...
    res = subprocess.Popen(['/opt/gcbm/moja.cli', '--config_file' if batch_config_file.endswith("cfg") else "--config", batch_config_file, '--config_provider', 'provider_config.json'], stdout=f, cwd='/gcbm_files/config')
    #e = time.time()
...
def get_blocks(staging_dir):
    ...
    localdomain = json.load(open(find_config_file(staging_config_dir, "LocalDomain"), "r"))
...
def upload_simulation_files(title, staging_dir, input_files):
    ...
    print(f"Client: uploading simulation files to {remote_simulation_dir} - in a real "
    ...
    for input_file in input_files:
        print(f"      {input_file}")
        remote_filename = os.path.join(remote_input_data_dir, os.path.basename(input_file))
...
def stage_files(title, gcbm_config, provider_config):
    ...
    if gcbm_config.endswith("cfg"):
        ...
        with open(working_gcbm_config_file, "w") as working_gcbm_config:
            for line in open(gcbm_config, "r"):
                ...
                shutil.copy(os.path.join(gcbm_config_dir, config_file), staging_config_dir)
    ...
    input_files = []
    working_provider_config_file = os.path.join(staging_config_dir, "provider_config.json")
    ...
    with update_json_file(working_provider_config_file) as working_provider_config:
        for provider_name, provider_settings in working_provider_config["Providers"].items():
            datasource_path = provider_settings.get("path")
            if datasource_path:
                input_files.append(os.path.abspath(os.path.join(os.path.dirname(provider_config), datasource_path)))
                provider_settings["path"] = "/".join((input_data_root, os.path.basename(datasource_path)))
            ...
            if layers:
                ...
                for layer in layers:
                    ...
                    abs_layer_path = os.path.abspath(os.path.join(original_provider_path, layer_path))
                    ...
                    if os.path.isfile(abs_layer_path):
                        ...
                        input_files.append(os.path.abspath(os.path.join(original_provider_path, layer_metadata_path)))
                        
                    layer["layer_path"] = "/".join((input_data_root, os.path.basename(layer_path)))
