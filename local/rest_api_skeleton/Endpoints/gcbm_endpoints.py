...
from Helpers.preprocess import DisturbanceConfig, launch_run, ClassifierConfig, MiscellaneousConfig
from Helpers.for_requests import title_query, miscellaneous_query, disturbance_query, classifier_query
...
class disturbance(Resource):
    def post(self):
        ...
        for file in disturbances.get("disturbances"):
            ...
            try:
                disturb = DisturbanceConfig(input_dir, file.filename, disturbances.get("attributes"))
                disturb()
...
class classifier(Resource):
    def post(self):
        ...
        for file in classifiers.get("classifiers"):
            ...
            try:
                classify = ClassifierConfig(input_dir, file.filename, classifiers.get("attributes"))
                classify()
            except Exception as e:
                return e
...
class miscellaneous(Resource):
    def post(self):
        ...
        for file in mis.get("miscellaneous"):
            ...
            try:
                miscel = MiscellaneousConfig(input_dir, file.filename, mis.get("attributes"))
                miscel()
            except Exception as e:
                return e
